...
// 물뿌리개 — 몸통·손잡이·주둥이·살수구
P.wateringCan = (q, X, Y) => {
  const c = '#6cb4c9', h2 = '#9ad5e4', d = '#3f7d92';
  P.shadow(q, X, Y, 7, 2);
  q(X - 5, Y - 9, 8, 9, c); q(X - 5, Y - 9, 8, 1, h2); q(X + 1, Y - 8, 2, 8, d);
  q(X - 5, Y - 11, 7, 2, h2);                                       // 어깨
  q(X - 4, Y - 15, 1, 5, d); q(X - 3, Y - 16, 4, 1, d); q(X + 1, Y - 15, 1, 4, d);   // 손잡이
  q(X + 3, Y - 10, 2, 2, c); q(X + 5, Y - 12, 2, 2, c);             // 주둥이 — 비스듬히 위로
  q(X + 7, Y - 14, 2, 2, h2); q(X + 8, Y - 17, 3, 3, h2); q(X + 9, Y - 14, 1, 1, d);  // 살수구
};
// 양동이 — 테두리와 손잡이
P.bucket = (q, X, Y, col) => {
  col = col || '#5aa9e6'; const d = shade(col, -26), h2 = shade(col, 22);
  P.shadow(q, X, Y, 5, 2);
  for (let i = 0; i < 8; i++){                    // 아래로 갈수록 좁아지는 통
    const w = 8 - (i >= 5 ? 2 : i >= 3 ? 1 : 0);
    q(X - Math.floor(w / 2), Y - 8 + i, w, 1, i === 0 ? h2 : col);
  }
  q(X + 1, Y - 7, 1, 6, d);
  q(X - 5, Y - 9, 10, 1, h2);                     // 테두리
  q(X - 5, Y - 12, 1, 3, '#8a8276'); q(X + 4, Y - 12, 1, 3, '#8a8276'); q(X - 4, Y - 13, 8, 1, '#8a8276');
};
// 씨앗 자루 — 위를 접어 둔 마대. 알갱이가 조금 흘러 있다
P.sack = (q, X, Y, col) => {
  col = col || '#d8c39a'; const d = shade(col, -22), h2 = shade(col, 16);
  P.shadow(q, X, Y, 6, 2);
  q(X - 5, Y - 9, 10, 9, col); q(X - 5, Y - 9, 3, 9, h2); q(X + 3, Y - 9, 2, 9, d);
  q(X - 4, Y - 12, 8, 3, shade(col, -8)); q(X - 4, Y - 12, 8, 1, h2);
  q(X - 2, Y - 13, 4, 1, d);
  q(X + 5, Y - 1, 2, 1, '#c9a24a'); q(X + 7, Y, 1, 1, '#c9a24a');      // 흘린 씨앗
};
// 화분 — 테를 두른 흙 그릇에 새싹 하나
P.pot = (q, X, Y, col) => {
  col = col || '#c9705a'; const d = shade(col, -24);
  P.shadow(q, X, Y, 4, 2);
  q(X - 3, Y - 6, 7, 6, col); q(X + 2, Y - 6, 2, 6, d); q(X - 4, Y - 8, 9, 2, shade(col, 14));
  q(X - 2, Y - 9, 5, 1, '#6a4c36');                                     // 흙
  q(X, Y - 12, 1, 3, '#4f8f4a'); q(X - 2, Y - 12, 2, 2, '#6fb567'); q(X + 1, Y - 13, 2, 2, '#6fb567');
};
// 지지대 — 말뚝 둘을 노끈으로 묶고 덩굴이 감았다
P.stakes = (q, X, Y, h) => {
  h = h || 18;
  P.shadow(q, X + 3, Y, 6, 2);
  [0, 7].forEach((dx, i) => {
    q(X + dx, Y - h - i, 1, h + i, '#a67a52'); q(X + dx, Y - h - i, 1, 2, '#c8a072');
    for (let v = 3; v < h; v += 5) q(X + dx - 1, Y - v, 3, 1, '#6fb567');   // 감긴 덩굴
  });
...
P.hen = (q, X, Y, col) => {
  col = col || '#fff6e9'; const d = shade(col, -16);
  P.shadow(q, X, Y, 5, 2);
  q(X - 4, Y - 6, 8, 5, col); q(X - 4, Y - 6, 8, 1, shade(col, 12)); q(X - 4, Y - 2, 8, 1, d);
  q(X - 6, Y - 7, 3, 4, d);                                  // 꼬리
  q(X + 2, Y - 10, 4, 5, col); q(X + 2, Y - 10, 4, 1, shade(col, 12));
  q(X + 3, Y - 12, 3, 2, '#e05545'); q(X + 4, Y - 13, 1, 1, '#e05545');    // 볏
  q(X + 5, Y - 8, 2, 2, '#f7b733');                          // 부리
  q(X + 4, Y - 9, 1, 1, '#2a2622');                          // 눈
...
function groundColor(kind, tx, ty, x, y){
  ...
  if (kind === 'soil'){
    // 이랑 한 골을 여섯 단으로 — 골 바닥에서 두렁 이마까지 밝아졌다 다시 어두워진다.
    // 세 단(전) 으로는 줄무늬로만 보였고, 여섯 단이면 흙이 둥글게 부풀어 보인다.
    const f = ((ty - 8.7) / 0.35) % 1;
    let c = f < 0.10 ? '#4a3524' : f < 0.24 ? '#674934' : f < 0.46 ? '#8a6546'
          : f < 0.62 ? '#a67c57' : f < 0.82 ? '#7d5a40' : '#5a4030';
    const n = hash(x, y, 12);
    if (n < 0.10) c = shade(c, -11);            // 흙덩이
    else if (n > 0.94) c = shade(c, 9);         // 마른 흙
    return c;
  }
  if (kind === 'water'){
    const e = ty - 10.8;
  ...
  for (let tx = 0; tx < PW; tx++) for (let ty = 0; ty < PD; ty++){
    for (let k = 0; k < 7; k++){
      ...
      if (kind === 'grass' || kind === 'garden'){
        ...
        // 밭 마당 — 흙덩이, 굴러다니는 지푸라기, 밟고 다닌 자국, 자갈
        if (r < 0.16) q(X, Y, 2, 1, '#8a6a48');
        else if (r < 0.24){ const c = hash(tx, ty, 80 + k) < 0.5 ? '#d8b358' : '#c9a24a'; q(X - 1, Y, 3, 1, c); q(X + 2, Y - 1, 1, 1, c); }
        else if (r < 0.30){ q(X - 1, Y, 3, 1, '#6f5334'); q(X, Y - 1, 1, 1, '#6f5334'); }   // 발자국
        else if (r < 0.335) P.rock(q, X, Y, 3);
      } else if (kind === 'soil'){
        // 이랑 — 두렁 이마에 볕, 고랑에 물 준 자국. 가끔 돌과 잡초가 섞인다
        const f2 = ((fy - 8.7) / 0.35) % 1;
        if (f2 < 0.2 && r < 0.5) q(X - 2, Y - 1, 5, 1, '#a07a56');                          // 두렁 이마
        else if (f2 > 0.55 && r < 0.28) q(X - 2, Y, 5, 1, '#4e3826');                        // 물 준 고랑
        else if (r < 0.33) q(X, Y, 2, 1, '#5c4230');
        else if (r < 0.36) P.rock(q, X, Y, 3);
        else if (r < 0.40) P.tuft(q, X, Y, '#5f8f4a');                                       // 잡초 한 포기
      }
    }
  }
...
function bGreenhouse(q){
  ...
  /* 유리 너머 — 선반 하나와 그 위의 화분 줄, 아래에는 큰 포기 몇. 전에는 초록 점을 흩뿌렸는데
     밖에서 보면 얼룩으로만 보였다. 모양을 주니 「안에 뭔가 자라고 있다」로 읽힌다.
     유리를 통해 보는 것이라 전부 반투명으로 얹는다. */
  const W2 = bx.w * S - 2;
  B.on(q, 'L', bx, 1, 12, W2, 1, () => 'rgba(150,118,78,0.55)');                 // 선반
  B.on(q, 'L', bx, 1, 6, W2, 6, (u, v) => {                                       // 선반 위 화분 줄
    const k = (u + 2) % 7;
    if (v >= 4) return k >= 1 && k <= 5 ? 'rgba(168,96,62,0.5)' : null;           // 화분
    if (k === 3) return 'rgba(64,132,68,0.5)';                                    // 줄기
    if ((k === 2 || k === 4) && v >= 1) return 'rgba(86,158,84,0.45)';             // 잎
    return null;
  });
  B.on(q, 'L', bx, 1, 13, W2, 3, (u, v) => {                                      // 선반 아래 큰 포기
    const k = (u + 4) % 9;
    if (k >= 2 && k <= 6 && v >= 3 - Math.abs(k - 4)) return 'rgba(70,140,70,0.42)';
    return null;
  });
  /* 지붕 유리 — 창틀 격자에 더해, 아래 화분이 비쳐 초록이 뭉쳐 보인다.
     농장에서 가장 넓은 면이라 여기가 밋밋하면 온실이 흰 덩어리로만 보인다.
     난수로 흩뿌리면 얼룩이 되어서, 사인 두 개를 겹쳐 덩어리로 뭉치게 했다. */
  B.roofFor(q, bx, { type: 'gable', rise: 10, over: 0.06, drop: 1, tex: f => (u, v) => {
    if (u % 8 === 0 || v % 5 === 0) return '#f4f8fa';                         // 창틀
    const g = Math.sin(u * 0.55) + Math.sin(v * 0.9 + u * 0.2);
    const base = g > 1.05 ? '#a6cdc0' : g > 0.45 ? '#bcdcdc' : (u - v) % 9 === 0 ? '#e6f3f8' : '#c9e4ee';
    return shade(base, lit(f));
  }, cap: ['#f4f8fa', '#9fb8c6'] });
}
function bShed(q){
...
VS.draw = function(env){
  ...
  // 미술관 앞뜰 — 조각, 가로등 (벤치는 뺐다 — 문 앞을 막고 서 있었다)
  ...
  // 광장 오른쪽 벤치(8.6, 4.5)는 뺐다 — 성 왼쪽 탑 앞을 가려 보여서(2026-09-15 부모 요청). 남은 벤치는 강가 하나
  ...
  add(8.32, 10.5, 0.2, 0.2, 14, q2 => P.pot(q2, ...at(8.42, 10.6)), 8);
  add(8.58, 10.58, 0.2, 0.2, 14, q2 => P.pot(q2, ...at(8.68, 10.68), '#b9895a'), 8);
  ...
  if (SPR.S.bird){
    add(5.55, 8.6, 0.2, 0.15, 22, q2 => { const b = at(5.6, 8.66); spr(q2, b[0] - 4, b[1] - 20, SPR.S.bird, SPR.PAL); }, 8);
    add(6.75, 8.6, 0.2, 0.15, 22, q2 => { const b = at(6.8, 8.66); spr(q2, b[0] - 4, b[1] - 19, SPR.S.bird, SPR.PAL); }, 8);
  }
  ...
  VS.labels = [
    ...
    { text: '업적전시실', href: '/honors.html', x: 5.45, y: 1.0, z: 82 },   // 2026-09-15 일기장 → 업적전시실(부모 요청). 집 그림(bDiary)은 그대로
...
function render(o){
  ...
  return {
    ...
    scare: VS.scare || null,                    // 허수아비 발밑(도트) — 그 뒤에는 작물을 안 심는다
