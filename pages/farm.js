  ...
  switch (F.kind){
    ...
    case 'fox': {                                                        // 레샤 인형 — 실물 사진을 보고
      /* 예전에는 서 있는 분홍 여우였다. 실물은 납작하게 엎드린 인형이고, 가장 큰 특징은
         **코에서 정수리로 곧게 이어진 분홍 줄**이다.
         분홍은 한 가지만 쓴다 — 귀·줄·몸·발이 모두 같은 색이고 그늘도 안 넣는다.
         얼굴만 크림색이라 그 위로 분홍 줄이 또렷하게 지나간다. */
      const cream = '#f4e9de', lite = '#fbf4ec';
      const pink = '#f0cfc9';
      const ink = '#2b2622', nose = '#9c5b2a', noseHi = '#c07f43';
      const rows = (list, col) => list.forEach(([y, x, w]) => oq(x, y, w, 1, col));

      // 귀 둘 — 머리보다 먼저 그린다(뒤에 놓인다). 통째로 분홍.
      rows([[2, 3, 8], [3, 1, 11], [4, 0, 13], [5, 0, 13], [6, 0, 13], [7, 1, 12], [8, 3, 9]], pink);
      rows([[0, 24, 6], [1, 23, 8], [2, 22, 9], [3, 21, 10], [4, 21, 10],
            [5, 21, 10], [6, 21, 10], [7, 22, 9], [8, 23, 7], [9, 25, 4]], pink);

      // 납작하게 벌린 앞발 둘과 가운데 몸 — 전부 같은 분홍 한 가지
      rows([[21, 1, 10], [22, 0, 12], [23, 0, 12], [24, 0, 12], [25, 1, 11], [26, 2, 9], [27, 4, 6]], pink);
      rows([[21, 21, 10], [22, 20, 12], [23, 20, 12], [24, 20, 12], [25, 20, 11], [26, 21, 9], [27, 22, 6]], pink);
      rows([[23, 9, 14], [24, 8, 16], [25, 8, 16], [26, 8, 16], [27, 9, 14], [28, 10, 12], [29, 12, 8]], pink);

      // 머리 — 옆으로 넓은 납작한 덩이. 아래는 몸으로 그대로 이어진다.
      rows([[7, 10, 12], [8, 7, 18], [9, 5, 22], [10, 4, 24], [11, 3, 26], [12, 3, 26],
            [13, 2, 28], [14, 2, 28], [15, 2, 28], [16, 2, 28], [17, 3, 26], [18, 3, 26],
            [19, 4, 24], [20, 5, 22], [21, 7, 18], [22, 8, 16], [23, 9, 14]], cream);
      rows([[8, 9, 14], [9, 7, 18], [10, 6, 20]], lite);                     // 이마에 빛

      // 코에서 정수리로 이어진 분홍 줄 — 귀와 똑같은 분홍 한 가지
      rows([[7, 13, 6], [8, 13, 6], [9, 13, 6], [10, 13, 6], [11, 13, 6], [12, 13, 6],
            [13, 13, 6], [14, 13, 6], [15, 13, 6], [16, 13, 6], [17, 14, 4], [18, 14, 4]], pink);

      // 눈 — 작고 까만 동그라미 둘 (얼굴 한가운데를 두고 좌우 대칭)
      [[8, 13], [19, 13]].forEach(([x, y]) => {
        oq(x + 1, y, 2, 1, ink); oq(x, y + 1, 4, 1, ink); oq(x + 1, y + 2, 2, 1, ink);
      });
      // 옆으로 긴 코 — 아래에 어두운 줄을 두면 입처럼 보여서 넣지 않는다
      oq(13, 18, 6, 1, nose); oq(12, 19, 8, 2, nose); oq(13, 21, 6, 1, nose);
      oq(14, 19, 4, 1, noseHi);
      break;
    }
