...
function paintFurniture(g, f, rot, A, t){
  ...
  switch (F.kind){
    ...
    case 'fox': {                                                        // 레샤 인형 — 실물 사진을 보고
      /* 예전에는 서 있는 분홍 여우였다. 실물은 납작하게 엎드린 인형이고, 가장 큰 특징은
         **코에서 정수리로 곧게 이어진 분홍 줄**이다. 눈과 코는 까망이 아니라 구릿빛 자수.
         귀는 둘 다 크다 — 왼쪽은 앞으로 접혀 눕고 오른쪽은 서 있다. */
      const cream = '#f4e9de', lite = '#fbf4ec', dim = '#e2d2c2';
      const pink = '#f0cfc9', pink2 = '#e0b5ae';
      const eye = '#9c5b2a', eyeHi = '#c07f43';
      const rows = (list, col) => list.forEach(([y, x, w]) => oq(x, y, w, 1, col));

      // 귀 둘 — 머리보다 먼저 그린다(뒤에 놓인다). 크기가 머리 반만 하다.
      rows([[2, 3, 8], [3, 1, 11], [4, 0, 13], [5, 0, 13], [6, 0, 13], [7, 1, 12], [8, 3, 9]], cream);
      rows([[4, 2, 8], [5, 1, 10], [6, 2, 8], [7, 4, 5]], pink);            // 접힌 귀 안쪽
      rows([[0, 24, 6], [1, 23, 8], [2, 22, 9], [3, 21, 10], [4, 21, 10],
            [5, 21, 10], [6, 21, 10], [7, 22, 9], [8, 23, 7], [9, 25, 4]], cream);
      rows([[2, 24, 5], [3, 23, 7], [4, 23, 7], [5, 23, 7], [6, 24, 5], [7, 25, 3]], pink);

      // 납작하게 벌린 앞발 둘 — 사진처럼 좌우로 크게 퍼진다
      rows([[21, 1, 10], [22, 0, 12], [23, 0, 12], [24, 0, 12], [25, 1, 11], [26, 2, 9], [27, 4, 6]], cream);
      rows([[21, 21, 10], [22, 20, 12], [23, 20, 12], [24, 20, 12], [25, 20, 11], [26, 21, 9], [27, 22, 6]], cream);
      rows([[25, 2, 9], [26, 3, 7], [25, 21, 9], [26, 22, 7]], dim);         // 발 아래 그늘
      // 가운데 몸 — 두 발 사이로 내민다
      rows([[23, 9, 14], [24, 8, 16], [25, 8, 16], [26, 8, 16], [27, 9, 14], [28, 10, 12], [29, 12, 8]], cream);
      rows([[27, 10, 12], [28, 11, 10], [29, 13, 6]], dim);

      // 머리 — 옆으로 넓은 납작한 덩이
      rows([[7, 10, 12], [8, 7, 18], [9, 5, 22], [10, 4, 24], [11, 3, 26], [12, 3, 26],
            [13, 2, 28], [14, 2, 28], [15, 2, 28], [16, 2, 28], [17, 3, 26], [18, 3, 26],
            [19, 4, 24], [20, 5, 22], [21, 7, 18], [22, 8, 16], [23, 9, 14]], cream);   // 아래는 몸으로 그대로 이어진다 — 여기서 끊으면 그 윤곽이 「입」처럼 보인다
      rows([[8, 9, 14], [9, 7, 18], [10, 6, 20]], lite);                     // 이마에 빛

      // 코에서 정수리로 이어진 분홍 줄 — 이 인형의 표식
      rows([[7, 13, 6], [8, 13, 6], [9, 13, 6], [10, 13, 6], [11, 13, 6], [12, 13, 6],
            [13, 13, 6], [14, 13, 6], [15, 13, 6], [16, 13, 6], [17, 14, 4], [18, 14, 4]], pink);
      rows([[9, 15, 2], [10, 15, 2], [11, 15, 2], [12, 15, 2], [13, 15, 2], [14, 15, 2]], pink2);

      // 구릿빛 자수 — 동그란 눈 둘. 네 귀퉁이를 깎아 네모가 아니라 동그라미로.
      [[7, 13], [20, 13]].forEach(([x, y]) => {
        oq(x + 1, y, 3, 1, eye); oq(x, y + 1, 5, 2, eye); oq(x + 1, y + 3, 3, 1, eye);
        oq(x + 1, y + 1, 2, 1, eyeHi);
      });
      // 옆으로 긴 코 — 아래에 어두운 줄을 두면 입처럼 보여서 넣지 않는다
      oq(13, 18, 6, 1, eye); oq(12, 19, 8, 2, eye); oq(13, 21, 6, 1, eye);
      oq(14, 19, 4, 1, eyeHi);
      break;
    }
