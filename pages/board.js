...
// 서울 기준이다 — 좌표는 일부러 구 단위로 뭉갰다(공개 저장소)(pages/index.js 의 WEATHER_AT · farm-rules.js 의 SKY_AT 과 같은 값).
// 최근 것은 예보 API 에 past_days 를 붙여 받고, 두 달이 넘은 날은 기록 보관 API 로 간다.
// 실패하면 도장 없이 그냥 저장된다 — 날씨는 덤이지, 일기를 막을 이유가 아니다.
const W_AT = 'latitude=37.53&longitude=127.08' +
  ...
  form.querySelector('.eSave').addEventListener('click', async () => {
    ...
    try {
      if (file) {
        ...
        const up = await uploadMedia(file, 'posts', POST_IMAGE_LIMIT, { capDim: PHOTO_CAP_DIM });
  ...
  $('#pSave').addEventListener('click', async () => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        const up = await uploadMedia(files[i], 'posts', POST_IMAGE_LIMIT, { capDim: PHOTO_CAP_DIM });
