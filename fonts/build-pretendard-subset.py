...
#
# 왜:
#   전에는 jsDelivr 의 「동적 서브셋」 CSS 를 썼다. 유니코드 구간마다 조각을 나눠 두고
#   화면에 나온 글자가 든 조각만 받는 방식인데, 첫 화면 한 장에 조각 열여섯 개가 붙어
#   330KB 가 왔다. 우리가 쓰는 글자는 정해져 있으니 한 벌로 자르면 왕복도 한 번이다.
#
# 남기는 것:
...
# (CDN 조각)로 가므로 티가 나지 않는다. 2026-09-04 에 세 벌을 잘라 재 봤다:
#     지금 목록 1447자 → 161,996 바이트
#     + DB 글자 + 낱자 1705자 → 162,336 바이트   (+340 바이트)
#     + 상용 2350자 전부 3048자 → 322,140 바이트  (+160KB)
# 가운데를 골랐다. 상용 전부는 값이 너무 비싸다.
#
# 조합 낱자(U+1100~U+11FF)를 꼭 넣어야 한다:
#   맥에서 친 글은 한글이 자모로 쪼개진 NFD 로 저장된다. works.title 39개 중 7개가
#   그랬다. 눈으로는 "친구"인데 실제로는 ᄎ+ᅵ+ᆫ+ᄀ+ᅮ 다섯 자라, 호환 낱자(U+3131~)
#   만 넣어 두면 이 글자들이 전부 CDN 조각으로 새어 나간다. 실제로 첫 화면에서
#   조각 두 개 50KB 를 그렇게 받고 있었다. 낱자 블록은 340 바이트면 들어온다.
#   common.js 의 escapeHTML 도 NFC 로 맞춰 두었지만, textContent 로 바로 넣는
#   자리가 남아 있어서 글꼴 쪽도 같이 막아 둔다.
#
# 굵기 축도 잘라 둔다. 원본은 45~930 인데 사이트가 쓰는 것은 400~900 뿐이라,
# 400 아래를 버리면 217KB → 158KB 가 된다.
#
# 여기 없는 글자는 어떻게 되나:
#   시스템 한글 글꼴로 간다. CDN 프리텐다드는 2026-09-04 에 걷어냈다 — 그물인 줄
#   알았는데 자른 벌에 있는 글자에도 조각을 받아 대서 50KB 를 그냥 버리고 있었다.
#   그러니 글이 늘면 이 스크립트를 다시 돌려 새 글자를 넣어 줘야 한다.
#
# 다시 만들려면:
...
# custom_tabs·event_meta·stamps 를 훑어 NFC 로 맞춘 뒤 뽑았다(362자).
# 글이 늘면 여기도 다시 뽑아야 한다 — 안 뽑으면 새 글자마다 CDN 조각 25KB 를 받는다.
DB_HANGUL = (
 "가각간감갑갔강같개거검것게겠경계고곡골곰곳과관광괜교구국군귤그근글금기길깃까깐꼬꼭꿈나날남내냠너널넘네녀녁년념노놀누눈뉴느는늘늦니다단담당대더덕던데도독돈돌"
 "동돼되될됩두드든들등디따때또띄라락래략량러럽레렐려렴렵로록롭료류르른를름릅리림릿마막만많말맛맞먼면명몇모목무문물미밀바반발밤밥방밭배백뱃법베변별보본볼봉부분"
...
chars |= {chr(c) for c in range(0x3131, 0x3164)}   # 호환 낱자 (ㄱ~ㅣ)
chars |= {chr(c) for c in range(0x1100, 0x1200)}   # 조합 낱자 — 아래 설명 참고
chars |= set('　、。「」『』·…—–―‘’“”₩％°※→←↑↓★☆♥♡✓✕±×÷≠≤≥∞')
chars |= set(DB_HANGUL)
