...
-- 2026-09-15 — 연주회장 녹음실(concert_rehearsals). 아이가 연습곡을 녹음해 무대에 「리허설」로 올린다.
-- ※ 아직 실제 DB 에 적용 전(적용 허락을 기다리는 중). 표가 없으면 연주회장은 녹음실 단추를 숨긴다.
-- 가족(로그인)만 넣고, 처음엔 가족만 본다. 부모가 is_public 을 켠 것만 손님에게 보인다. 녹음 파일은 목소리 일기와 같은 suayona/voice/ 폴더.
create table if not exists public.concert_rehearsals (
  id bigint generated by default as identity primary key,
  author text not null check (author in ('sua', 'yona')),
  title text not null check (char_length(title) between 1 and 40),
  audio_url text not null check (audio_url like 'https://ifiemaypzjwdrljmmkgb.supabase.co/storage/v1/object/public/event-images/suayona/voice/%'),
  audio_secs smallint not null check (audio_secs between 1 and 60),
  is_public boolean not null default false,
  written_by uuid default auth.uid(),
  created_at timestamptz not null default now()
);
alter table public.concert_rehearsals enable row level security;
drop policy if exists "read rehearsals" on public.concert_rehearsals;
create policy "read rehearsals" on public.concert_rehearsals for select
  using (is_public or (select my_role()) is not null);
drop policy if exists "family adds rehearsals" on public.concert_rehearsals;
create policy "family adds rehearsals" on public.concert_rehearsals for insert
  with check ((select my_role()) = 'parent'
    or ((select my_role()) = 'child' and author = (select my_author_key()) and written_by = (select auth.uid()) and is_public = false));
drop policy if exists "parent edits rehearsals" on public.concert_rehearsals;
create policy "parent edits rehearsals" on public.concert_rehearsals for update
  using ((select my_role()) = 'parent') with check ((select my_role()) = 'parent');
drop policy if exists "family drops rehearsals" on public.concert_rehearsals;
create policy "family drops rehearsals" on public.concert_rehearsals for delete
  using ((select my_role()) = 'parent' or ((select my_role()) = 'child' and written_by = (select auth.uid())));
create index if not exists concert_rehearsals_created_idx on public.concert_rehearsals (created_at desc);
