...
-- 2026-09-15 — 연주회장 박수(concert_claps). 작품 박수(work_claps)와 따로 센다(부모 요청). 틀·규칙은 work_claps 와 같다.
create table if not exists public.concert_claps (
  id bigint generated by default as identity primary key,
  work_id bigint not null references public.works(id) on delete cascade,
  created_at timestamptz not null default now()
);
create index if not exists concert_claps_work_idx on public.concert_claps (work_id);
alter table public.concert_claps enable row level security;
drop policy if exists "anyone reads concert claps" on public.concert_claps;
create policy "anyone reads concert claps" on public.concert_claps for select using (true);
drop policy if exists "anyone claps concerts" on public.concert_claps;
create policy "anyone claps concerts" on public.concert_claps for insert with check (true);
drop policy if exists "parent clears concert claps" on public.concert_claps;
create policy "parent clears concert claps" on public.concert_claps for delete to authenticated using ((select my_role()) = 'parent');
create or replace function public.concert_claps_rate_guard() returns trigger
language plpgsql security definer set search_path = public as $$
begin
  new.created_at := now();   -- 손님이 적어 보낸 시각은 버린다
  if (select count(*) from concert_claps where created_at > now() - interval '1 minute') >= 30
     or (select count(*) from concert_claps where created_at > now() - interval '1 hour') >= 300 then
    raise exception '박수가 너무 빨리 쏟아지고 있어요. 잠시 뒤에 다시 해 주세요.';
  end if;
  return new;
end $$;
revoke execute on function public.concert_claps_rate_guard() from public, anon, authenticated;
drop trigger if exists concert_claps_rate on public.concert_claps;
create trigger concert_claps_rate before insert on public.concert_claps for each row execute function public.concert_claps_rate_guard();
-- 영상마다 박수 수 — 줄을 다 받지 않고 서버가 센다(연주회장 탭을 열 때만 부른다)
create or replace function public.concert_clap_counts() returns table(work_id bigint, n bigint)
language sql stable set search_path = public as $$ select work_id, count(*) from concert_claps group by work_id $$;
-- 2026-09-15 — 연주회장 응원 스티커(concert_cheers). 스티커 종류(0~5)만 받고 글은 받지 않는다 — 부모가 따로 살필 것이 없다.
-- (같은 밤 부모 요청으로 화면에서 스티커 기능을 뺐다. 표·함수는 아직 DB 에 남아 있다 — 지울지는 부모에게 묻는 중)
create table if not exists public.concert_cheers (
  id bigint generated by default as identity primary key,
  work_id bigint not null references public.works(id) on delete cascade,
  kind smallint not null check (kind between 0 and 5),
  created_at timestamptz not null default now()
);
create index if not exists concert_cheers_work_idx on public.concert_cheers (work_id);
alter table public.concert_cheers enable row level security;
drop policy if exists "anyone reads concert cheers" on public.concert_cheers;
create policy "anyone reads concert cheers" on public.concert_cheers for select using (true);
drop policy if exists "anyone cheers concerts" on public.concert_cheers;
create policy "anyone cheers concerts" on public.concert_cheers for insert with check (kind between 0 and 5);
drop policy if exists "parent clears concert cheers" on public.concert_cheers;
create policy "parent clears concert cheers" on public.concert_cheers for delete to authenticated using ((select my_role()) = 'parent');
create or replace function public.concert_cheers_rate_guard() returns trigger
language plpgsql security definer set search_path = public as $$
begin
  new.created_at := now();   -- 손님이 적어 보낸 시각은 버린다
  if (select count(*) from concert_cheers where created_at > now() - interval '1 minute') >= 30
     or (select count(*) from concert_cheers where created_at > now() - interval '1 hour') >= 300 then
    raise exception '스티커가 너무 빨리 붙고 있어요. 잠시 뒤에 다시 해 주세요.';
  end if;
  return new;
end $$;
revoke execute on function public.concert_cheers_rate_guard() from public, anon, authenticated;
drop trigger if exists concert_cheers_rate on public.concert_cheers;
create trigger concert_cheers_rate before insert on public.concert_cheers for each row execute function public.concert_cheers_rate_guard();
create or replace function public.concert_cheer_counts() returns table(work_id bigint, kind smallint, n bigint)
language sql stable set search_path = public as $$ select work_id, kind, count(*) from concert_cheers group by work_id, kind $$;
