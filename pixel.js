...
// 위경도 한 점이 어느 묶음에 드는지.
//
// 위의 그림 격자로 되짚으면 안 된다. 저것은 「그림」이라 해협을 줄여 그렸고,
// 줄 수도 25줄인데 예전 코드는 30줄로 셌다. 그래서 제주(위도 33.2~33.6)가
// 배열 밖으로 나가 늘 null 이 되어, 제주 일정을 열아홉 개나 찍어 두고도
// 지도가 안 칠해지고 있었다 (2026-09-04 에 찾음).
//
// 그래서 그림과 따로, 실제 경계에 가까운 상자를 위에서부터 본다. 겹치는 자리는
// 순서로 가른다 — 전북을 전남보다 먼저 보는 것은 남원(35.42)이 전남 상자에도
// 들기 때문이고, 강원을 위아래로 나눈 것은 경기가 여주까지 동쪽으로 나오기
// 때문이다. 잘 아는 도시 서른여덟 곳으로 재서 서른여덟 곳 다 맞는다.
const KOREA_BOX = [
  // [묶음, 위도 최소, 위도 최대, 경도 최소, 경도 최대]
  ['E', 32.90, 33.75, 125.90, 127.10], // 제주
  ['S', 34.40, 35.65, 127.75, 129.60], // 경남·부산·울산
  ['B', 35.35, 36.05, 126.30, 127.95], // 전북
  ['J', 33.90, 35.45, 125.90, 127.75], // 전남·광주
  ['N', 35.60, 37.20, 127.90, 129.70], // 경북·대구
  ['C', 36.00, 37.05, 125.90, 128.10], // 충청·대전·세종
  ['W', 37.40, 38.70, 127.60, 129.50], // 강원 북쪽
  ['W', 37.00, 37.40, 127.80, 129.50], // 강원 남쪽
  ['G', 36.90, 38.40, 125.90, 127.80], // 경기·서울·인천
];

function koreaRegionAt(lat, lng){
  if (!(lat > 0) || !(lng > 0)) return null;
  for (let i = 0; i < KOREA_BOX.length; i++) {
    const b = KOREA_BOX[i];
    if (lat >= b[1] && lat <= b[2] && lng >= b[3] && lng <= b[4]) return b[0];
  }
  return null;
}
